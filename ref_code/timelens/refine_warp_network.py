...
class RefineWarp(warp_network.Warp, fusion_network.Fusion):
    """
    inherit both warp net and fusion (synthesis) net
    """
    ...
    def __init__(self):
        ...
        """
        define warp net, synth net and refinement net
        """
    ...
    def run_refine_warp(self, example):
        ...
        """
        add synthesis and optical flow warped images to the example struct

        calls parent class functions
        """
        ...
        """
        store middle fusion, run it through the flow refinement net to get residual flow 
        (4 channels for before and after residual flow)
        """

        (after_residual, before_residual) = th.chunk(residual, 2, dim=1)
        residual = th.cat([after_residual, before_residual], dim=0)
        """
        move things around, get residual flows cat'ed
        """

        refined, refined_invalid = warp.backwarp_2d(
            source=_pack_images_for_second_warping(example),
            y_displacement=residual[:, 0, ...],         # where the moving around comes in
            x_displacement=residual[:, 1, ...],
        )
        """
        warp images through warp.backwarp_2d
        warp images acc to residual flow
        """
...
"""
takes in synthesis images and optical flow inital guesses, refine both
"""